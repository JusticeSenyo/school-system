...
const EMAIL_API_BASE =
  ...
  'https://schoolmasterhub.vercel.app'; // your mail API host
...
export default function SupportChat() {
  ...
  useEffect(() => { try { localStorage.setItem(STORAGE_KEYS.draft, message); } catch {} }, [message]);
  useEffect(() => { try { localStorage.setItem(STORAGE_KEYS.category, category); } catch {} }, [category]);
  useEffect(() => { try { localStorage.setItem(STORAGE_KEYS.open, open ? '1' : '0'); } catch {} }, [open]);

  const meta = useMemo(() => {
    const role = user?.userType ?? user?.role ?? '';
    const school = user?.schoolName ?? user?.school?.name ?? user?.school_name ?? '';
    const schoolId = user?.schoolId ?? user?.school_id ?? user?.school?.id ?? '';
    const email = user?.email ?? user?.EMAIL ?? '';
    const name = user?.name ?? user?.full_name ?? user?.FULL_NAME ?? '';
    return {
      name, role, email, school, schoolId,
  ...
  const sendMail = async () => {
    ...
    try {
      const res = await fetch(`${EMAIL_API_BASE}/api/send-email`, {
        ...
        body: JSON.stringify({
          to: [SUPPORT_TO],                // <- array (matches API)
          subject,                         // <- required
          message: buildText(),            // <- single message string
          fromName: meta.name || 'SMH User'
        }),
      });
